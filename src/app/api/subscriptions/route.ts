...
import { cacheGet, cacheSet, filterHash, subsKey } from "@/lib/cache";
...
export const GET = withErrorHandling(async (req: NextRequest) => {
  ...
  // Cache-aside per filter combination (Stage 9); writers invalidate.
  const key = subsKey(user.id, filterHash({ status, verdict, account_id, cursor }));
